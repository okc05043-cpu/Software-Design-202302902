...
export default function TopBar({ user, onLogout, isMobile }) {
  ...
  return (
    <div style={{ background: '#2563eb', color: 'white', padding: isMobile ? '12px 16px' : '14px 20px', marginBottom: 20 }}>
      <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
        <span style={{ fontWeight: 'bold', fontSize: isMobile ? 13 : 17 }}>
          {isMobile ? '학생 관리 시스템' : '학생 성적 및 상담 관리 시스템'}
        </span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {!isMobile && <span style={{ fontSize: 13 }}>{user.name} ({roleLabel})</span>}
